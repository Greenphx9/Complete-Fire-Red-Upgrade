...
u8 ScriptGiveMon(u16 species, u8 level, u16 item, unusedArg u32 unused1, unusedArg u32 customGivePokemon, unusedArg u8 ballType)
{
	...
	if (customGivePokemon != 0)
	{
		...
		GiveMonNatureAndAbility(&mon, nature, abilityNum, shiny ? TRUE : IsMonShiny(&mon), FALSE, FALSE);
		if(abilityNum == 2)
			mon.hiddenAbility = TRUE;
		HealMon(&mon);
...
void GetRandomBuyableMon(void)
{
	...
	while(gSpecialSpeciesFlags[species].smogonNationalDexOUBan 
	|| CanSpeciesEvolve(species) == TRUE
	|| gSpecialSpeciesFlags[species].randomizerBan
	|| gSpecialSpeciesFlags[species].smartWild
	|| gSpecialSpeciesFlags[species].setPerfectXIVs
